#!/usr/bin/env python3
"""Fix HTML issues and optimize loading for aipu-pages."""
import re
from pathlib import Path

BASE = Path(__file__).resolve().parent
ASSETS = '/aipu-pages/assets'
...
VOICE_AGENTS_CSS = r'''<style id="aipu-voice-fixes">
...
def optimize(html: str) -> str:
    ...
    html = re.sub(r'<link rel="preload" href="https://aipu\.com/_next/static/chunks/[^"]+\.css"[^>]*>', '', html)
    html = re.sub(r'<link rel="stylesheet" href="https://aipu\.com/_next/static/chunks/[^"]+\.css"[^>]*>', '', html)

    # Remove font preload (bundled in local CSS path or skip)
    html = re.sub(r'<link rel="preload" href="https://aipu\.com/_next/static/media/[^"]+\.woff2"[^>]*>', '', html)
    ...
    html = html.replace(
        'src="https://app.aiprofessionalsuniversity.com/_next/image?url=%2Flogo-aipu.png&w=384&q=75"',
        f'src="{ASSETS}/logo-aipu.png"',
    )

    # Inject or refresh perf head. The existing block spans from the first
    # dns-prefetch link through the end of the aipu-static-fixes style, with
    # several preconnect/stylesheet links in between, so match across all of it.
    if 'aipu-static-fixes' in html:
        html = re.sub(
            r'<link rel="dns-prefetch" href="https://aipu-images\.b-cdn\.net">.*?<style id="aipu-static-fixes">.*?</style>',
    ...
    # the Discover grid (see aipu-static-fixes CSS) renders evenly on every page.
    html = re.sub(
...
def post_process(filename: str, html: str) -> str:
    if filename == 'create-voice-agents.html':
        ...
        if 'aipu-voice-fixes' not in html:
