...
OMNI = 'https://app.aiprofessionalsuniversity.com'
ASSET_VER = '20260607h'

MARKETING_CSS = f'''<link rel="dns-prefetch" href="https://omnirogue-images.b-cdn.net">
...
MARKETING_HEAD_BLOCK = re.compile(
    r'<link rel="dns-prefetch" href="https://(?:omnirogue-images|aipu-images)\.b-cdn\.net">\s*'
    r'<link rel="dns-prefetch" href="https://images\.unsplash\.com">\s*'
    r'<link rel="preconnect" href="https://(?:omnirogue-images|aipu-images)\.b-cdn\.net" crossorigin>\s*'
    r'(?:<link rel="stylesheet" href="/aipu-pages/assets/main[12]\.css">\s*)*'
...
def fix_marketing_head(html: str) -> str:
    """Marketing pages use local main1/main2 CSS (remote AIPU chunks 404)."""
    html = re.sub(
        r'<link rel="dns-prefetch" href="https://(?:aipu|app\.aiprofessionalsuniversity)\.com">\s*'
        r'<link rel="preconnect" href="https://(?:aipu|app\.aiprofessionalsuniversity)\.com" crossorigin>\s*'
    ...
    html = MARKETING_HEAD_BLOCK.sub('', html)
    html = re.sub(
    ...
    html = re.sub(
        r'<link rel="preload" href="https://(?:app\.)?aiprofessionalsuniversity\.com/_next/static/chunks/[^"]+\.css"[^>]*>\s*',
        '',
        html,
    )
...
def _optimize_marketing_common(html: str) -> str:
    ...
    html = html.replace('href="https://app.aiprofessionalsuniversity.com/register"', f'href="{LOCAL}/checkout.html"')
    html = html.replace(f'href="{OMNI}/plans"', f'href="{LOCAL}/checkout.html"')
    html = html.replace('href="https://app.aiprofessionalsuniversity.com/plans"', f'href="{LOCAL}/checkout.html"')
    ...
    html = re.sub(
        r'srcset="/_next/image\?url=%2Flogo-aipu\.png[^"]*"',
        f'srcset="{ASSETS}/logo-aipu.png 1x, {ASSETS}/logo-aipu.png 2x"',
        html,
    )
    html = re.sub(
        r'srcset="/aipu-pages/assets/logo-aipu\.png[^"]*"',
        f'srcset="{ASSETS}/logo-aipu.png 1x, {ASSETS}/logo-aipu.png 2x"',
        html,
    )
    html = re.sub(
        r'src="https://aipu\.com/_next/image\?url=%2Flogo-aipu\.png[^"]*"',
        f'src="{ASSETS}/logo-aipu.png"',
        html,
    )
    html = re.sub(
        r'src="/_next/image\?url=%2Flogo-aipu\.png[^"]*"',
        f'src="{ASSETS}/logo-aipu.png"',
        html,
    )
