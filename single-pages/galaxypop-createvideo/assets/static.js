...
(function () {
  ...
  var navMap = {
    'AI Video': '/single-pages/galaxypop-createvideo/createvideo.html',
    'Image': '/single-pages/galaxypop-createvideo/create-image.html',
    'Audio': '/single-pages/galaxypop-createvideo/create-audio.html',
    'Music': '/single-pages/galaxypop-createvideo/create-music.html',
    'Upscale': '/single-pages/galaxypop-createvideo/create-upscale.html',
    'OmniReels': '/single-pages/galaxypop-createvideo/create-omnireels.html',
    'Podcast': '/single-pages/galaxypop-createvideo/create-podcast.html',
    'AI Chat': '/single-pages/galaxypop-createvideo/create-ai-chat.html',
    'Voice Agents': '/single-pages/galaxypop-createvideo/create-voice-agents.html'
  };
  ...
  var __LANDER_BASE='/single-pages/galaxypop-createvideo';
  ...
  function omniBasePath() {
    ...
    if (path.indexOf('/single-pages/galaxypop-createvideo/') >= 0) return '/single-pages/galaxypop-createvideo';
    if (path.indexOf('/aipu-pages/') >= 0) return '/aipu-pages';
    var trimmed = path.replace(/\/$/, '');
    if (trimmed) return trimmed;
    return fromNav || fromScript || '/single-pages/galaxypop-createvideo';
  }
  ...
  // ============================================================
  // Billing pop-up — the AIPU checkout/billing page rendered as an
  // in-page modal (isolated <iframe>) instead of an external checkout.
  // Exposed as window.OmniBilling so every "upgrade / unlock" surface
  // (generation-ready card, exit popup, premium gates) opens it.
  // ============================================================
  var OmniBilling = (function () {
    var overlay = null, frame = null, loaded = false;

    function billingUrl() {
      return omniBasePath() + '/billing.' + omniPageExt();
    }

    function injectStyles() {
      if (document.getElementById('omni-billing-styles')) return;
      var st = document.createElement('style');
      st.id = 'omni-billing-styles';
      st.textContent = [
        '#omni-billing-overlay{position:fixed;inset:0;z-index:2147483646;display:none;align-items:center;justify-content:center;padding:0;background:rgba(4,4,10,.86);backdrop-filter:blur(10px);animation:omniBillFade .25s ease}',
        '#omni-billing-overlay.omni-billing-open{display:flex}',
        '#omni-billing-shell{position:relative;width:100%;height:100%;background:#070812}',
        '@media(min-width:768px){#omni-billing-shell{width:min(960px,96vw);height:min(94vh,1040px);border-radius:22px;overflow:hidden;box-shadow:0 50px 130px rgba(0,0,0,.7),0 0 0 1px rgba(139,92,255,.28),0 0 70px rgba(139,92,255,.18)}}',
        '#omni-billing-frame{width:100%;height:100%;border:0;display:block;background:#070812}',
        '#omni-billing-close{position:absolute;top:.7rem;right:.7rem;z-index:3;width:2.4rem;height:2.4rem;border-radius:999px;background:rgba(10,8,22,.7);color:#fff;font-size:1.5rem;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(6px);border:1px solid rgba(255,255,255,.18);transition:background .15s,transform .15s}',
        '#omni-billing-close:hover{background:rgba(139,92,255,.55);transform:scale(1.07)}',
        '@keyframes omniBillFade{from{opacity:0}}'
      ].join('');
    ...
    function build() {
      ...
      overlay.id = 'omni-billing-overlay';
      overlay.innerHTML =
        '<div id="omni-billing-shell">' +
          '<button type="button" id="omni-billing-close" aria-label="Close">&times;</button>' +
          '<iframe id="omni-billing-frame" title="AIPU membership" allow="payment"></iframe>' +
        '</div>';
      document.body.appendChild(overlay);
      frame = overlay.querySelector('#omni-billing-frame');
      overlay.querySelector('#omni-billing-close').addEventListener('click', close);
    ...
    function ensureLoaded() {
      build();
      if (!loaded) { frame.src = billingUrl(); loaded = true; }
    }

    function isOpen() { return !!overlay && overlay.classList.contains('omni-billing-open'); }

    function open() {
      ensureLoaded();
      // collapse the generation-ready card so we don't stack modals
      var gen = document.getElementById('omni-gen-overlay');
      if (gen) gen.style.display = 'none';
      overlay.classList.add('omni-billing-open');
      document.documentElement.style.overflow = 'hidden';
    }

    function close() {
      if (overlay) overlay.classList.remove('omni-billing-open');
      document.documentElement.style.overflow = '';
    }

    // Warm the iframe in the background so the pop-up is instant the
    // moment a generation finishes.
    function preload() { ensureLoaded(); }

    return { open: open, close: close, preload: preload, isOpen: isOpen };
  })();
  window.OmniBilling = OmniBilling;

  // Open the billing pop-up; fall back to a register/checkout URL only if
  // the pop-up is somehow unavailable. Returns true when handled in-page.
  function omniGoBilling() {
    if (window.OmniBilling) { window.OmniBilling.open(); return true; }
    return false;
  }

  // Pre-warm the billing iframe once the page is idle.
  if (typeof window.requestIdleCallback === 'function') {
    window.requestIdleCallback(function () { try { OmniBilling.preload(); } catch (e) {} });
  } else {
    setTimeout(function () { try { OmniBilling.preload(); } catch (e) {} }, 1500);
  }

  // Global nav targets: Home -> index, Pricing/Plans/Login/Register -> pop-up.
  (function wireTopNav() {
    var HOME = omniHomeUrl();
    var CHECKOUT = omniCheckoutUrl();
    var base = omniBasePath();
    var ext = omniPageExt();
    // Conversion link/label tests: Pricing/Plans + every auth surface (Login,
    // Sign in, Create Account, Register, Get Started). On a single-page flow the
    // billing pop-up IS the checkout, so all of these open it in-page.
    var CONVERSION_LABEL = /^(pricing|plans|log\s?in|login|sign\s?in|sign\s?up|register|create account|get started|start free|try free|upgrade|go unlimited|get unlimited)$/i;
    var CONVERSION_HREF = /(\/plans\/?$|\/pricing\/?$|\/register\b|\/login\b|\/signup\b|\/sign-up\b|\/billing\b|billing\.(?:html|php)\b|checkout\.(?:html|php)\b)/i;
    document.querySelectorAll('a').forEach(function (a) {
      var label = a.textContent.replace(/\s+/g, ' ').trim();
      var href = a.getAttribute('href') || '';
      // Home link in the top nav
      if (label === 'Home') { a.setAttribute('href', HOME); }
      // Pricing / Plans / Login / Register / Create Account etc. (top nav, footer,
      // body CTAs) all open the in-page billing pop-up instead of leaving the site.
      if (CONVERSION_LABEL.test(label) || CONVERSION_HREF.test(href)) {
        a.setAttribute('href', CHECKOUT);
        a.addEventListener('click', function (e) {
          if (omniGoBilling()) e.preventDefault();
        });
      }
      if (label === 'Help') {
  ...
  (function initGenerateFlow() {
    ...
    function buildPreviewHTML(p) {
      if (p.kind === 'video' && p.src) {
        return '<div class="omni-gen-preview"><video src="' + p.src + '" autoplay muted loop playsinline class="omni-gen-media"></video>' +
          '<span class="omni-gen-preview-badge">Preview</span></div>';
      }
      if (p.kind === 'image' && p.src) {
        return '<div class="omni-gen-preview"><img src="' + p.src + '" alt="Your generation" class="omni-gen-media">' +
          '<span class="omni-gen-preview-badge">Preview</span></div>';
      }
      if (p.kind === 'audio') {
        var poster = p.poster ? '<img src="' + p.poster + '" alt="" class="omni-gen-media omni-gen-media--dim">' : '';
        return '<div class="omni-gen-preview omni-gen-preview--audio">' + poster +
          '<div class="omni-gen-audio-ui"><div class="omni-gen-wave">' + waveBars(28) + '</div>' +
          '<button type="button" class="omni-gen-play" aria-label="Play preview" id="omni-gen-play"><svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><path d="M5 5a2 2 0 0 1 3.008-1.728l11.997 6.998a2 2 0 0 1 .003 3.458l-12 7A2 2 0 0 1 5 19z"></path></svg></button></div>' +
          (p.src ? '<audio id="omni-gen-audio" src="' + p.src + '" preload="none"></audio>' : '') +
          '<span class="omni-gen-preview-badge">Preview</span></div>';
      }
      if (p.kind === 'music') {
        var cover = p.poster ? '<img src="' + p.poster + '" alt="" class="omni-gen-media omni-gen-media--dim">' : '';
        return '<div class="omni-gen-preview omni-gen-preview--audio">' + cover +
          '<div class="omni-gen-audio-ui"><div class="omni-gen-wave omni-gen-wave--music">' + waveBars(34) + '</div>' +
          '<button type="button" class="omni-gen-play" aria-label="Play preview"><svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><path d="M5 5a2 2 0 0 1 3.008-1.728l11.997 6.998a2 2 0 0 1 .003 3.458l-12 7A2 2 0 0 1 5 19z"></path></svg></button></div>' +
          '<span class="omni-gen-preview-badge">Preview</span></div>';
      }
      if (p.kind === 'chat') {
        return '<div class="omni-gen-preview omni-gen-preview--chat">' +
          '<div class="omni-gen-chat-bubble"><p>Your AI response is ready — unlock unlimited chat across 140+ models.</p></div>' +
          '<span class="omni-gen-preview-badge">Preview</span></div>';
      }
      return '';
    }
    ...
    function buildModelListHTML(models, activeIdx, done) {
      return models.map(function (name, i) {
        var cls = 'omni-gen-model';
        if (done) cls += ' omni-gen-model--done';
        else if (i === activeIdx) cls += ' omni-gen-model--active';
        var icon = done ? '✓' : (i === activeIdx ? '◉' : '○');
        return '<div class="' + cls + '"><span class="omni-gen-model-icon">' + icon + '</span><span>' + name + '</span></div>';
      }).join('');
    }
    ...
    if (!document.getElementById('omni-gen-styles')) {
      ...
      style.textContent = [
        '#omni-gen-overlay{position:fixed;inset:0;z-index:99999;display:none;align-items:center;justify-content:center;padding:1.25rem;background:rgba(4,4,10,.82);backdrop-filter:blur(12px);animation:omniGenFadeIn .3s ease}',
        '#omni-gen-card{position:relative;max-width:30rem;width:100%;max-height:92vh;overflow-y:auto;padding:0;border-radius:1.5rem;border:1px solid rgba(139,92,255,.22);background:linear-gradient(160deg,rgba(28,18,52,.97) 0%,rgba(10,8,20,.99) 100%);box-shadow:0 32px 80px rgba(0,0,0,.65),0 0 60px rgba(139,92,255,.12),inset 0 1px 0 rgba(255,255,255,.06)}',
        ...
        '.omni-gen-inner{position:relative;padding:1.6rem 1.6rem 1.4rem}',
        '.omni-gen-badge{display:inline-flex;align-items:center;gap:.5rem;padding:.35rem .85rem;border-radius:999px;font-size:.7rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#c4b5fd;background:rgba(139,92,255,.12);border:1px solid rgba(139,92,255,.25);margin-bottom:1rem}',
        '.omni-gen-head{display:flex;align-items:center;gap:1rem;margin-bottom:1rem}',
        '.omni-gen-icon{width:3rem;height:3rem;border-radius:1rem;display:flex;align-items:center;justify-content:center;font-size:1.5rem;background:linear-gradient(135deg,rgba(139,92,255,.2),rgba(56,189,248,.1));border:1px solid rgba(255,255,255,.08);flex-shrink:0}',
        '#omni-gen-status{font-size:1.15rem;font-weight:700;color:#fafafa;margin:0;line-height:1.35}',
        '#omni-gen-sub{font-size:.8rem;color:rgba(161,161,170,.9);margin:.35rem 0 0}',
        '.omni-gen-spinner-wrap{display:flex;justify-content:center;margin:1.5rem 0}',
        '.omni-gen-spinner{width:2.75rem;height:2.75rem;border:3px solid rgba(139,92,255,.15);border-top-color:#8b5cf6;border-radius:50%;animation:omniGenSpin .75s linear infinite}',
        // Preview media
        '.omni-gen-preview{position:relative;margin:.25rem 0 1.15rem;border-radius:1rem;overflow:hidden;border:1px solid rgba(255,255,255,.08);aspect-ratio:16/10;background:#0c0a18;animation:omniGenSlideUp .4s ease}',
        '.omni-gen-media{width:100%;height:100%;object-fit:cover;display:block}',
        '.omni-gen-media--dim{position:absolute;inset:0;opacity:.45}',
        '.omni-gen-preview--audio{display:flex;align-items:center;justify-content:center;background:linear-gradient(150deg,rgba(46,16,101,.9),rgba(15,23,42,.95))}',
        '.omni-gen-audio-ui{position:relative;z-index:2;display:flex;flex-direction:column;align-items:center;gap:1rem}',
        '.omni-gen-wave{display:flex;align-items:center;gap:3px;height:48px}',
        ...
        '.omni-gen-play{width:3rem;height:3rem;border-radius:50%;border:1px solid rgba(255,255,255,.2);background:rgba(139,92,255,.35);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:transform .15s,background .15s}',
        '.omni-gen-play:hover{transform:scale(1.08);background:rgba(139,92,255,.6)}',
        '.omni-gen-preview-badge{position:absolute;top:.6rem;left:.6rem;z-index:3;padding:.2rem .6rem;border-radius:.5rem;font-size:.6rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#fff;background:rgba(0,0,0,.55);backdrop-filter:blur(4px)}',
        '.omni-gen-preview--chat{display:flex;align-items:center;justify-content:center;padding:1.5rem;background:linear-gradient(150deg,rgba(46,16,101,.9),rgba(15,23,42,.95))}',
        '.omni-gen-chat-bubble{max-width:88%;padding:1rem 1.15rem;border-radius:1rem 1rem 1rem .35rem;background:rgba(139,92,255,.18);border:1px solid rgba(139,92,255,.28);color:#e4e4e7;font-size:.85rem;line-height:1.5}',
        '#omni-gen-models{display:flex;flex-direction:column;gap:.45rem;margin:0 0 1rem;text-align:left}',
        '.omni-gen-model{display:flex;align-items:center;gap:.55rem;padding:.45rem .65rem;border-radius:.65rem;font-size:.78rem;color:rgba(161,161,170,.85);background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.04);transition:all .25s ease}',
        '.omni-gen-model--active{color:#fafafa;background:rgba(139,92,255,.15);border-color:rgba(139,92,255,.35);box-shadow:0 0 20px rgba(139,92,255,.12)}',
        '.omni-gen-model--done{color:#86efac;border-color:rgba(34,197,94,.25);background:rgba(34,197,94,.08)}',
        '.omni-gen-model-icon{width:1rem;text-align:center;font-size:.7rem;flex-shrink:0}',
        '#omni-gen-claim{display:block;width:100%;padding:.95rem 1.25rem;border:none;border-radius:.9rem;font-size:.95rem;font-weight:700;color:#fff;cursor:pointer;background:linear-gradient(135deg,#8b5cf6,#6d28d9);box-shadow:0 8px 28px rgba(139,92,255,.4);transition:transform .15s,box-shadow .15s}',
        '#omni-gen-claim:hover{transform:translateY(-2px);box-shadow:0 12px 36px rgba(139,92,255,.5)}',
        '.omni-gen-claim-sub{text-align:center;font-size:.72rem;color:rgba(161,161,170,.8);margin:.7rem 0 0}',
        // state: processing vs ready
        '#omni-gen-card:not(.omni-gen-ready) .omni-gen-ready-only{display:none}',
        '#omni-gen-card.omni-gen-ready .omni-gen-spinner-wrap{display:none}',
    ...
    function ensureModal() {
      ...
      modal.innerHTML =
        '<div id="omni-gen-card">' +
          ...
          '<div class="omni-gen-inner">' +
            '<div class="omni-gen-badge"><span>' + studio.icon + '</span><span>' + studio.label + '</span></div>' +
            '<div class="omni-gen-head">' +
              '<div class="omni-gen-icon">' + studio.icon + '</div>' +
              '<div><p id="omni-gen-status">Processing...</p><p id="omni-gen-sub">Rendering your ' + studio.ready + '</p></div>' +
            '</div>' +
            '<div class="omni-gen-spinner-wrap"><div class="omni-gen-spinner"></div></div>' +
            '<div id="omni-gen-models"></div>' +
            '<div class="omni-gen-ready-only">' +
              '<div id="omni-gen-preview-slot"></div>' +
              '<button type="button" id="omni-gen-claim">Click here to unlock instant access + unlimited usage on 140+ top models</button>' +
              '<p class="omni-gen-claim-sub">Includes ' + studio.label + ' &amp; every OmniRogue studio &middot; cancel anytime</p>' +
            '</div>' +
          '</div>' +
        '</div>';
      ...
      modal.addEventListener('click', function (e) {
        if (e.target === modal) {
          if (billingAutoTimer) { clearTimeout(billingAutoTimer); billingAutoTimer = null; }
    ...
    var billingAutoTimer = null;

    // Open the billing pop-up (no external checkout). Cancels any pending
    // auto-open timer first so we never re-open after a manual dismiss.
    ...
    function runGenerationFlow() {
      ...
      var sub = m.querySelector('#omni-gen-sub');
      var slot = m.querySelector('#omni-gen-preview-slot');
      var modelsEl = m.querySelector('#omni-gen-models');
      ...
      setTimeout(function () {
        ...
        processing = false;
        syncGenerateState();
        // A beat after the finished result is revealed, surface the billing
        // pop-up so "generation ready" flows straight into the upgrade offer.
        if (window.OmniBilling) {
          if (billingAutoTimer) clearTimeout(billingAutoTimer);
          billingAutoTimer = setTimeout(function () {
            billingAutoTimer = null;
            window.OmniBilling.open();
          }, 1700);
        }
      }, 3000);
    }
    ...
    function wirePreviewPlay(slot) {
      var btn = slot.querySelector('.omni-gen-play');
  ...
  (function initStudioConversion() {
    ...
    if (!document.getElementById('omni-conv-styles')) {
      ...
      s.textContent = [
        ...
        // (exit-intent now opens the billing pop-up — no standalone exit card)
        // social proof toast
    ...
    // ---- 3. Exit-intent -> open the billing pop-up (once per session) ----
    // The standalone exit-intent offer card was removed; exit-intent now surfaces
    // the same in-page billing pop-up the Generate button opens, so there is a
    // single conversion surface (and a single offer/KK wiring) across the flow.
    (function initExitPopup() {
      ...
      function show() {
        if (shown) return;
        // Don't stack on top of the billing pop-up if it's already open
        // (e.g. opened by a just-finished generation).
        if (window.OmniBilling && window.OmniBilling.isOpen && window.OmniBilling.isOpen()) return;
        shown = true;
        try { sessionStorage.setItem(KEY, '1'); } catch (e) {}
        // Open the billing pop-up; fall back to the register/checkout link.
        if (!omniGoBilling()) {
          var url = omniRegisterCheckoutUrl();
          if (url) window.location.href = url;
        }
      ...
      // Mobile fallback: fast scroll up near the top (back-gesture surrogate).
