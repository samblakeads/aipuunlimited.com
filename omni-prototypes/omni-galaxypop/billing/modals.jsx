...
// ---------- Exit-intent offer ----------
function ExitIntentModal({ open, onClose, onClaim }) {
  if (!open) return null;
  const gpt = window.MODELS.gptimg;
  const nano = { glyph: "◗", bg: "#F5C24B", fg: "#1a1206" };
  return (
    <div className="scrim" onClick={onClose}>
      <div className="modal exit-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="exit-pad">
          ...
          <h3 className="exit-h">Wait — before you go!</h3>
          <p className="exit-sub">Get <b className="hl">500 free generations</b> of <b>GPT Image 2</b> and <b>Nano Banana Pro</b> — for just <b>$14.99/month</b>.</p>
          <div className="exit-models">
            <span className="exit-model"><ModelTile m={gpt} size={26} /> GPT Image 2</span>
            <span className="exit-model"><ModelTile m={nano} size={26} /> Nano Banana Pro</span>
          </div>
          <button className="btn btn-bluefill btn-lg btn-full" onClick={onClaim}>Claim 500 free generations <Icon name="ArrowRight" size={17} /></button>
          <button className="exit-no" onClick={onClose}>No thanks, I'll pass on free generations</button>
          <div className="exit-foot"><Icon name="RefreshCw" size={12} /> Cancel anytime · <Icon name="ShieldCheck" size={12} /> 14-day money-back</div>
        </div>
      </div>
    </div>
  );
...
Object.assign(window, { CheckoutModal, MonthlyWarning, InfoModal, ExitIntentModal });
