// AIPU static pages — shared interactions
(function () {
  'use strict';

  var navMap = {
    'AI Video': '/aipu-multistep/aipufull-plans-v3-49/createvideo.html',
    'Image': '/aipu-multistep/aipufull-plans-v3-49/create-image.html',
    'Audio': '/aipu-multistep/aipufull-plans-v3-49/create-audio.html',
    'Music': '/aipu-multistep/aipufull-plans-v3-49/create-music.html',
    'Upscale': '/aipu-multistep/aipufull-plans-v3-49/create-upscale.html',
    'AIPU Reels': '/aipu-multistep/aipufull-plans-v3-49/create-omnireels.html',
    'Podcast': '/aipu-multistep/aipufull-plans-v3-49/create-podcast.html',
    'AI Chat': '/aipu-multistep/aipufull-plans-v3-49/create-ai-chat.html',
    'Voice Agents': '/aipu-multistep/aipufull-plans-v3-49/create-voice-agents.html'
  };
  ...
  var __LANDER_BASE = '/aipu-multistep/aipufull-plans-v3-49';
  ...
  function omniBasePath() {
    ...
    if (m && m[1]) return m[1];
    if (path.indexOf('/aipu-newpages/') >= 0) return '/aipu-newpages';
    if (path.indexOf('/aipu-multistep/aipufull-plans-v3-49/') >= 0) return '/aipu-pages';
    if (path.indexOf('/aipu-pages/') >= 0) return '/aipu-pages';
    var trimmed = path.replace(/\/$/, '');
    if (trimmed) return trimmed;
    return fromNav || fromScript || '/aipu-pages';
  }
  ...
  (function initLibraryDropdown() {
    ...
    if (!document.getElementById('aipu-library-styles')) {
      var style = document.createElement('style');
      style.id = 'aipu-library-styles';
      style.textContent = [
        '.aipu-library-wrap{position:relative}',
        '.aipu-library-menu{position:absolute;top:calc(100% + 10px);left:50%;transform:translateX(-50%);min-width:17.5rem;z-index:200;padding:.45rem;border-radius:1rem;border:1px solid rgba(139,92,255,.22);background:linear-gradient(165deg,rgba(22,16,42,.98) 0%,rgba(10,8,20,.99) 100%);box-shadow:0 24px 60px rgba(0,0,0,.55),0 0 40px rgba(139,92,255,.1),inset 0 1px 0 rgba(255,255,255,.05);opacity:0;visibility:hidden;pointer-events:none;transition:opacity .18s ease,transform .18s ease,visibility .18s}',
        '.aipu-library-menu.aipu-library-open{opacity:1;visibility:visible;pointer-events:auto;transform:translateX(-50%) translateY(0)}',
        '.aipu-library-menu-header{display:flex;align-items:center;gap:.45rem;padding:.55rem .75rem .45rem;font-size:.62rem;font-weight:700;letter-spacing:.14em;color:rgba(161,161,170,.85);border-bottom:1px solid rgba(255,255,255,.06);margin-bottom:.25rem}',
        '.aipu-library-menu-header svg{width:.95rem;height:.95rem;color:#a78bfa;flex-shrink:0}',
        '.aipu-library-item{display:flex;align-items:flex-start;gap:.75rem;padding:.7rem .75rem;border-radius:.75rem;text-decoration:none;color:inherit;transition:background .15s ease}',
        '.aipu-library-item:hover{background:rgba(139,92,255,.12)}',
        '.aipu-library-icon{width:2.35rem;height:2.35rem;border-radius:.7rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;background:rgba(139,92,255,.14);border:1px solid rgba(139,92,255,.22)}',
        '.aipu-library-icon svg{width:1.1rem;height:1.1rem;color:#c4b5fd}',
        '.aipu-library-title{font-size:.9rem;font-weight:700;color:#fafafa;line-height:1.25;margin:0}',
        '.aipu-library-desc{font-size:.72rem;color:rgba(161,161,170,.9);margin:.2rem 0 0;line-height:1.35}',
        'button[aria-controls="aipu-library-menu"]{cursor:pointer}'
      ].join('');
    ...
    if (!triggers.length || document.getElementById('aipu-library-menu')) return;

    var menu = document.createElement('div');
    menu.id = 'aipu-library-menu';
    menu.setAttribute('role', 'menu');
    menu.className = 'aipu-library-menu';
    menu.innerHTML =
      '<div class="aipu-library-menu-header">' +
        ...
        '<span>LIBRARY</span>' +
      '</div>' +
      '<a class="aipu-library-item" role="menuitem" href="' + gptUrl + '">' +
        '<div class="aipu-library-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/></svg></div>' +
        '<div><p class="aipu-library-title">GPT Library</p><p class="aipu-library-desc">Ready-made GPTs &amp; AI assistants</p></div>' +
      '</a>' +
      '<a class="aipu-library-item" role="menuitem" href="' + promptUrl + '">' +
        '<div class="aipu-library-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 7v14"/><path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z"/></svg></div>' +
        '<div><p class="aipu-library-title">Prompt Library</p><p class="aipu-library-desc">1,000+ battle-tested prompts by category</p></div>' +
      '</a>';

    var wrap = triggers[0].parentElement;
    if (wrap) wrap.classList.add('aipu-library-wrap');
    (wrap || document.body).appendChild(menu);

    function setOpen(open) {
      menu.classList.toggle('aipu-library-open', open);
    ...
    triggers.forEach(function (btn) {
      btn.setAttribute('aria-controls', 'aipu-library-menu');
      btn.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        setOpen(!menu.classList.contains('aipu-library-open'));
      });
    });
  ...
  (function initLibraryUpgradeGate() {
    ...
    if (!document.getElementById('aipu-lib-upgrade-styles')) {
      var style = document.createElement('style');
      style.id = 'aipu-lib-upgrade-styles';
      style.textContent = [
        '#aipu-lib-upgrade-overlay{position:fixed;inset:0;z-index:99999;display:none;align-items:center;justify-content:center;padding:1.25rem;background:rgba(4,4,10,.82);backdrop-filter:blur(12px);animation:omniGenFadeIn .3s ease}',
        '#aipu-lib-upgrade-card{position:relative;max-width:28rem;width:100%;padding:0;border-radius:1.5rem;border:1px solid rgba(139,92,255,.22);background:linear-gradient(160deg,rgba(28,18,52,.97) 0%,rgba(10,8,20,.99) 100%);box-shadow:0 32px 80px rgba(0,0,0,.65),0 0 60px rgba(139,92,255,.12),inset 0 1px 0 rgba(255,255,255,.06)}',
        '.aipu-lib-upgrade-glow{position:absolute;inset:-1px;border-radius:1.5rem;background:linear-gradient(135deg,rgba(139,92,255,.35),rgba(56,189,248,.15),rgba(139,92,255,.2));opacity:.45;pointer-events:none}',
        '.aipu-lib-upgrade-inner{position:relative;padding:1.6rem 1.6rem 1.4rem}',
        '.aipu-lib-upgrade-badge{display:inline-flex;align-items:center;gap:.5rem;padding:.35rem .85rem;border-radius:999px;font-size:.7rem;font-weight:700;letter-spacing:.04em;color:#86efac;background:rgba(34,197,94,.1);border:1px solid rgba(34,197,94,.28);margin-bottom:1rem}',
        '.aipu-lib-upgrade-head{display:flex;align-items:center;gap:1rem;margin-bottom:1rem}',
        '.aipu-lib-upgrade-icon{width:3rem;height:3rem;border-radius:1rem;display:flex;align-items:center;justify-content:center;font-size:1.5rem;background:linear-gradient(135deg,rgba(139,92,255,.2),rgba(56,189,248,.1));border:1px solid rgba(255,255,255,.08);flex-shrink:0}',
        '#aipu-lib-upgrade-title{font-size:1.15rem;font-weight:700;color:#fafafa;margin:0;line-height:1.35}',
        '#aipu-lib-upgrade-sub{font-size:.85rem;color:rgba(212,212,216,.92);margin:.5rem 0 0;line-height:1.5}',
        '.aipu-lib-upgrade-price{margin:1.15rem 0;padding:.85rem 1rem;border-radius:.85rem;background:rgba(139,92,255,.1);border:1px solid rgba(139,92,255,.2);font-size:.82rem;color:#e4e4e7;line-height:1.5}',
        '.aipu-lib-upgrade-price strong{color:#c4b5fd;font-weight:700}',
        '#aipu-lib-upgrade-cta{display:block;width:100%;padding:.95rem 1.25rem;border:none;border-radius:.9rem;font-size:.95rem;font-weight:700;color:#fff;cursor:pointer;background:linear-gradient(135deg,#8b5cf6,#6d28d9);box-shadow:0 8px 28px rgba(139,92,255,.4);transition:transform .15s,box-shadow .15s}',
        '#aipu-lib-upgrade-cta:hover{transform:translateY(-2px);box-shadow:0 12px 36px rgba(139,92,255,.5)}',
        '.aipu-lib-upgrade-foot{text-align:center;font-size:.72rem;color:rgba(161,161,170,.8);margin:.7rem 0 0}',
        '#aipu-lib-upgrade-close{position:absolute;top:.85rem;right:.85rem;width:2rem;height:2rem;border:none;border-radius:.5rem;background:rgba(255,255,255,.06);color:#a1a1aa;cursor:pointer;font-size:1.1rem;line-height:1}',
        '#aipu-lib-upgrade-close:hover{background:rgba(255,255,255,.12);color:#fff}'
      ].join('');
      document.head.appendChild(style);
    }

    var overlay = document.createElement('div');
    overlay.id = 'aipu-lib-upgrade-overlay';
    overlay.innerHTML =
      '<div id="aipu-lib-upgrade-card">' +
        '<div class="aipu-lib-upgrade-glow"></div>' +
        '<button type="button" id="aipu-lib-upgrade-close" aria-label="Close">&times;</button>' +
        '<div class="aipu-lib-upgrade-inner">' +
          '<div class="aipu-lib-upgrade-badge">✓ Text &amp; GPT are always unlimited</div>' +
          '<div class="aipu-lib-upgrade-head">' +
            '<div class="aipu-lib-upgrade-icon">' + icon + '</div>' +
            '<div>' +
              '<p id="aipu-lib-upgrade-title">Upgrade to access ' + noun + '</p>' +
              '<p id="aipu-lib-upgrade-sub">To access ' + noun + ', upgrade today!</p>' +
            '</div>' +
          '</div>' +
          '<p class="aipu-lib-upgrade-price">Plans starting at <strong>$14.99</strong> — or unlock <strong>unlimited</strong> on our higher plans!</p>' +
          '<button type="button" id="aipu-lib-upgrade-cta">Upgrade today</button>' +
          '<p class="aipu-lib-upgrade-foot">' + label + ' &middot; cancel anytime</p>' +
        '</div>' +
      '</div>';
    ...
    overlay.querySelector('#aipu-lib-upgrade-cta').addEventListener('click', function () {
      window.location.href = checkoutUrl;
    });
    overlay.querySelector('#aipu-lib-upgrade-close').addEventListener('click', hideUpgradeModal);
  ...
  (function initLocalUploads() {
    var STORAGE_KEY = 'aipu-local-uploads';
  ...
  (function initGenerateFlow() {
    ...
    var STUDIO = {
      ...
      omnireels: { key: 'omnireels', label: 'AIPU Reels', icon: '📱', ready: 'reel', kind: 'video' },
    ...
    var STUDIO_MODELS = {
      ...
      omnireels: ['AIPU Reels Studio', 'Kling 3', 'Veo 3.1', 'Seedance 2.0', 'CapCut AI'],
    ...
    function buildPreviewHTML(p) {
      if (p.kind === 'video' && p.src) {
        return '<div class="aipu-gen-preview"><video src="' + p.src + '" autoplay muted loop playsinline class="aipu-gen-media"></video>' +
          '<span class="aipu-gen-preview-badge">Preview</span></div>';
      }
      if (p.kind === 'image' && p.src) {
        return '<div class="aipu-gen-preview"><img src="' + p.src + '" alt="Your generation" class="aipu-gen-media">' +
          '<span class="aipu-gen-preview-badge">Preview</span></div>';
      }
      if (p.kind === 'audio') {
        var poster = p.poster ? '<img src="' + p.poster + '" alt="" class="aipu-gen-media aipu-gen-media--dim">' : '';
        return '<div class="aipu-gen-preview aipu-gen-preview--audio">' + poster +
          '<div class="aipu-gen-audio-ui"><div class="aipu-gen-wave">' + waveBars(28) + '</div>' +
          '<button type="button" class="aipu-gen-play" aria-label="Play preview" id="aipu-gen-play"><svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><path d="M5 5a2 2 0 0 1 3.008-1.728l11.997 6.998a2 2 0 0 1 .003 3.458l-12 7A2 2 0 0 1 5 19z"></path></svg></button></div>' +
          (p.src ? '<audio id="aipu-gen-audio" src="' + p.src + '" preload="none"></audio>' : '') +
          '<span class="aipu-gen-preview-badge">Preview</span></div>';
      }
      if (p.kind === 'music') {
        var cover = p.poster ? '<img src="' + p.poster + '" alt="" class="aipu-gen-media aipu-gen-media--dim">' : '';
        return '<div class="aipu-gen-preview aipu-gen-preview--audio">' + cover +
          '<div class="aipu-gen-audio-ui"><div class="aipu-gen-wave aipu-gen-wave--music">' + waveBars(34) + '</div>' +
          '<button type="button" class="aipu-gen-play" aria-label="Play preview"><svg width="18" height="18" viewBox="0 0 24 24" fill="#fff"><path d="M5 5a2 2 0 0 1 3.008-1.728l11.997 6.998a2 2 0 0 1 .003 3.458l-12 7A2 2 0 0 1 5 19z"></path></svg></button></div>' +
          '<span class="aipu-gen-preview-badge">Preview</span></div>';
      }
      if (p.kind === 'chat') {
        return '<div class="aipu-gen-preview aipu-gen-preview--chat">' +
          '<div class="aipu-gen-chat-bubble"><p>Your AI response is ready — unlock unlimited chat across 140+ models.</p></div>' +
          '<span class="aipu-gen-preview-badge">Preview</span></div>';
      }
      return '';
    }

    function buildModelListHTML(models, activeIdx, done) {
      return models.map(function (name, i) {
        var cls = 'aipu-gen-model';
        if (done) cls += ' aipu-gen-model--done';
        else if (i === activeIdx) cls += ' aipu-gen-model--active';
        var icon = done ? '✓' : (i === activeIdx ? '◉' : '○');
        return '<div class="' + cls + '"><span class="aipu-gen-model-icon">' + icon + '</span><span>' + name + '</span></div>';
      }).join('');
    }
    ...
    function waveBars(n) {
      ...
      var out = '';
      for (var i = 0; i < n; i++) {
        out += '<span class="aipu-gen-bar" style="height:' + (heights[i % heights.length]) + '%;animation-delay:' + (i * 0.05) + 's"></span>';
      }
      return out;
    }

    if (!document.getElementById('aipu-gen-styles')) {
      var style = document.createElement('style');
      style.id = 'aipu-gen-styles';
      style.textContent = [
        '#aipu-gen-overlay{position:fixed;inset:0;z-index:99999;display:none;align-items:center;justify-content:center;padding:1.25rem;background:rgba(4,4,10,.82);backdrop-filter:blur(12px);animation:omniGenFadeIn .3s ease}',
        '#aipu-gen-card{position:relative;max-width:30rem;width:100%;max-height:92vh;overflow-y:auto;padding:0;border-radius:1.5rem;border:1px solid rgba(139,92,255,.22);background:linear-gradient(160deg,rgba(28,18,52,.97) 0%,rgba(10,8,20,.99) 100%);box-shadow:0 32px 80px rgba(0,0,0,.65),0 0 60px rgba(139,92,255,.12),inset 0 1px 0 rgba(255,255,255,.06)}',
        '.aipu-gen-glow{position:absolute;inset:-1px;border-radius:1.5rem;background:linear-gradient(135deg,rgba(139,92,255,.35),rgba(56,189,248,.15),rgba(139,92,255,.2));opacity:.45;pointer-events:none}',
        '.aipu-gen-inner{position:relative;padding:1.6rem 1.6rem 1.4rem}',
        '.aipu-gen-badge{display:inline-flex;align-items:center;gap:.5rem;padding:.35rem .85rem;border-radius:999px;font-size:.7rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#c4b5fd;background:rgba(139,92,255,.12);border:1px solid rgba(139,92,255,.25);margin-bottom:1rem}',
        '.aipu-gen-head{display:flex;align-items:center;gap:1rem;margin-bottom:1rem}',
        '.aipu-gen-icon{width:3rem;height:3rem;border-radius:1rem;display:flex;align-items:center;justify-content:center;font-size:1.5rem;background:linear-gradient(135deg,rgba(139,92,255,.2),rgba(56,189,248,.1));border:1px solid rgba(255,255,255,.08);flex-shrink:0}',
        '#aipu-gen-status{font-size:1.15rem;font-weight:700;color:#fafafa;margin:0;line-height:1.35}',
        '#aipu-gen-sub{font-size:.8rem;color:rgba(161,161,170,.9);margin:.35rem 0 0}',
        '.aipu-gen-spinner-wrap{display:flex;justify-content:center;margin:1.5rem 0}',
        '.aipu-gen-spinner{width:2.75rem;height:2.75rem;border:3px solid rgba(139,92,255,.15);border-top-color:#8b5cf6;border-radius:50%;animation:omniGenSpin .75s linear infinite}',
        // Preview media
        '.aipu-gen-preview{position:relative;margin:.25rem 0 1.15rem;border-radius:1rem;overflow:hidden;border:1px solid rgba(255,255,255,.08);aspect-ratio:16/10;background:#0c0a18;animation:omniGenSlideUp .4s ease}',
        '.aipu-gen-media{width:100%;height:100%;object-fit:cover;display:block}',
        '.aipu-gen-media--dim{position:absolute;inset:0;opacity:.45}',
        '.aipu-gen-preview--audio{display:flex;align-items:center;justify-content:center;background:linear-gradient(150deg,rgba(46,16,101,.9),rgba(15,23,42,.95))}',
        '.aipu-gen-audio-ui{position:relative;z-index:2;display:flex;flex-direction:column;align-items:center;gap:1rem}',
        '.aipu-gen-wave{display:flex;align-items:center;gap:3px;height:48px}',
        '.aipu-gen-bar{width:3px;border-radius:99px;background:linear-gradient(to top,#8b5cf6,#c4b5fd);animation:omniGenWave 1s ease-in-out infinite alternate}',
        '.aipu-gen-wave--music .aipu-gen-bar{background:linear-gradient(to top,#38bdf8,#a78bfa)}',
        '.aipu-gen-play{width:3rem;height:3rem;border-radius:50%;border:1px solid rgba(255,255,255,.2);background:rgba(139,92,255,.35);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:transform .15s,background .15s}',
        '.aipu-gen-play:hover{transform:scale(1.08);background:rgba(139,92,255,.6)}',
        '.aipu-gen-preview-badge{position:absolute;top:.6rem;left:.6rem;z-index:3;padding:.2rem .6rem;border-radius:.5rem;font-size:.6rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#fff;background:rgba(0,0,0,.55);backdrop-filter:blur(4px)}',
        '.aipu-gen-preview--chat{display:flex;align-items:center;justify-content:center;padding:1.5rem;background:linear-gradient(150deg,rgba(46,16,101,.9),rgba(15,23,42,.95))}',
        '.aipu-gen-chat-bubble{max-width:88%;padding:1rem 1.15rem;border-radius:1rem 1rem 1rem .35rem;background:rgba(139,92,255,.18);border:1px solid rgba(139,92,255,.28);color:#e4e4e7;font-size:.85rem;line-height:1.5}',
        '#aipu-gen-models{display:flex;flex-direction:column;gap:.45rem;margin:0 0 1rem;text-align:left}',
        '.aipu-gen-model{display:flex;align-items:center;gap:.55rem;padding:.45rem .65rem;border-radius:.65rem;font-size:.78rem;color:rgba(161,161,170,.85);background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.04);transition:all .25s ease}',
        '.aipu-gen-model--active{color:#fafafa;background:rgba(139,92,255,.15);border-color:rgba(139,92,255,.35);box-shadow:0 0 20px rgba(139,92,255,.12)}',
        '.aipu-gen-model--done{color:#86efac;border-color:rgba(34,197,94,.25);background:rgba(34,197,94,.08)}',
        '.aipu-gen-model-icon{width:1rem;text-align:center;font-size:.7rem;flex-shrink:0}',
        '#aipu-gen-claim{display:block;width:100%;padding:.95rem 1.25rem;border:none;border-radius:.9rem;font-size:.95rem;font-weight:700;color:#fff;cursor:pointer;background:linear-gradient(135deg,#8b5cf6,#6d28d9);box-shadow:0 8px 28px rgba(139,92,255,.4);transition:transform .15s,box-shadow .15s}',
        '#aipu-gen-claim:hover{transform:translateY(-2px);box-shadow:0 12px 36px rgba(139,92,255,.5)}',
        '.aipu-gen-claim-sub{text-align:center;font-size:.72rem;color:rgba(161,161,170,.8);margin:.7rem 0 0}',
        // state: processing vs ready
        '#aipu-gen-card:not(.aipu-gen-ready) .aipu-gen-ready-only{display:none}',
        '#aipu-gen-card.aipu-gen-ready .aipu-gen-spinner-wrap{display:none}',
        '@keyframes omniGenSpin{to{transform:rotate(360deg)}}',
        '@keyframes omniGenFadeIn{from{opacity:0}to{opacity:1}}',
        '@keyframes omniGenSlideUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}',
    ...
    function ensureModal() {
      ...
      modal.id = 'aipu-gen-overlay';
      modal.innerHTML =
        '<div id="aipu-gen-card">' +
          '<div class="aipu-gen-glow"></div>' +
          '<div class="aipu-gen-inner">' +
            '<div class="aipu-gen-badge"><span>' + studio.icon + '</span><span>' + studio.label + '</span></div>' +
            '<div class="aipu-gen-head">' +
              '<div class="aipu-gen-icon">' + studio.icon + '</div>' +
              '<div><p id="aipu-gen-status">Processing...</p><p id="aipu-gen-sub">Rendering your ' + studio.ready + '</p></div>' +
            '</div>' +
            '<div class="aipu-gen-spinner-wrap"><div class="aipu-gen-spinner"></div></div>' +
            '<div id="aipu-gen-models"></div>' +
            '<div class="aipu-gen-ready-only">' +
              '<div id="aipu-gen-preview-slot"></div>' +
              '<button type="button" id="aipu-gen-claim">Click here to unlock instant access + unlimited usage on 140+ top models</button>' +
              '<p class="aipu-gen-claim-sub">Includes ' + studio.label + ' &amp; every AIPU studio &middot; cancel anytime</p>' +
            '</div>' +
          '</div>' +
        '</div>';
      document.body.appendChild(modal);
      modal.querySelector('#aipu-gen-claim').addEventListener('click', function () {
        var url = omniRegisterCheckoutUrl();
        if (url) window.location.href = url;
      });
      // Close on backdrop click
      modal.addEventListener('click', function (e) {
        if (e.target === modal) { modal.style.display = 'none'; teardownPreview(); }
      });
      return modal;
    }

    function teardownPreview() {
      var slot = modal && modal.querySelector('#aipu-gen-preview-slot');
      if (slot) {
    ...
    function runGenerationFlow() {
      ...
      var m = ensureModal();
      var card = m.querySelector('#aipu-gen-card');
      var status = m.querySelector('#aipu-gen-status');
      var sub = m.querySelector('#aipu-gen-sub');
      var slot = m.querySelector('#aipu-gen-preview-slot');
      var modelsEl = m.querySelector('#aipu-gen-models');
      var models = STUDIO_MODELS[studio.key] || STUDIO_MODELS.video;
      var activeModel = 0;

      if (modelCycleTimer) { clearInterval(modelCycleTimer); modelCycleTimer = null; }

      card.classList.remove('aipu-gen-ready');
      status.textContent = 'Processing...';
      sub.textContent = 'Rendering your ' + studio.ready + ' with our top models';
      slot.innerHTML = '';
      modelsEl.innerHTML = buildModelListHTML(models, 0, false);
      card.scrollTop = 0;
      m.style.display = 'flex';

      modelCycleTimer = setInterval(function () {
        activeModel = (activeModel + 1) % models.length;
        modelsEl.innerHTML = buildModelListHTML(models, activeModel, false);
      }, 550);

      setTimeout(function () {
        if (modelCycleTimer) { clearInterval(modelCycleTimer); modelCycleTimer = null; }
        card.classList.add('aipu-gen-ready');
        status.textContent = 'Congratulations, your ' + studio.ready + ' is ready!';
        sub.textContent = 'Unlock instant access + unlimited usage on 140+ top models.';
        modelsEl.innerHTML = buildModelListHTML(models, 0, true);
        var preview = collectPreview();
        if (studio.kind === 'chat') preview.kind = 'chat';
        slot.innerHTML = buildPreviewHTML(preview);
        wirePreviewPlay(slot);
        processing = false;
        syncGenerateState();
      }, 3000);
    }

    function wirePreviewPlay(slot) {
      var btn = slot.querySelector('.aipu-gen-play');
  ...
  (function initStudioConversion() {
    var path = window.location.pathname;
    var isCreate = path.indexOf('/create') >= 0 || /create(video|-[a-z]+)\.html/.test(path);
    if (!isCreate) return;
    ...
    var STUDIO_DEF = {
      ...
      omnireels: { key: 'omnireels', label: 'AIPU Reels', verb: 'made a reel', models: ['AIPU Reels Studio', 'Kling 3', 'Veo 3.1', 'Seedance 2.0', 'CapCut AI'] },
    ...
    function goCheckout() { window.location.href = checkoutUrl; }

    // Shared styles for studio dropdowns, exit popup and social proof toast.
    if (!document.getElementById('aipu-conv-styles')) {
      var s = document.createElement('style');
      s.id = 'aipu-conv-styles';
      s.textContent = [
        // dropdown shell (matches .aipu-library-menu language)
        '.aipu-dd{position:fixed;z-index:9998;min-width:14rem;max-width:20rem;max-height:60vh;overflow-y:auto;padding:.4rem;border-radius:1rem;border:1px solid rgba(139,92,255,.22);background:linear-gradient(165deg,rgba(22,16,42,.99) 0%,rgba(10,8,20,.99) 100%);box-shadow:0 24px 60px rgba(0,0,0,.6),0 0 40px rgba(139,92,255,.1),inset 0 1px 0 rgba(255,255,255,.05);opacity:0;transform:translateY(6px);pointer-events:none;transition:opacity .16s ease,transform .16s ease}',
        '.aipu-dd.aipu-dd-open{opacity:1;transform:translateY(0);pointer-events:auto}',
        '.aipu-dd-head{padding:.5rem .65rem .4rem;font-size:.6rem;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:rgba(161,161,170,.85);border-bottom:1px solid rgba(255,255,255,.06);margin-bottom:.25rem}',
        '.aipu-dd-item{display:flex;align-items:center;gap:.6rem;width:100%;text-align:left;padding:.55rem .65rem;border:none;background:none;border-radius:.6rem;color:#e4e4e7;font-size:.82rem;font-weight:500;cursor:pointer;transition:background .14s ease}',
        '.aipu-dd-item:hover{background:rgba(139,92,255,.12)}',
        '.aipu-dd-item .aipu-dd-check{width:1rem;flex-shrink:0;color:#a78bfa;font-size:.85rem;text-align:center;opacity:0}',
        '.aipu-dd-item.aipu-dd-sel .aipu-dd-check{opacity:1}',
        '.aipu-dd-item .aipu-dd-name{flex:1;min-width:0}',
        '.aipu-dd-lock{font-size:.6rem;font-weight:700;letter-spacing:.04em;padding:.12rem .4rem;border-radius:.35rem;color:#fcd34d;background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.25);flex-shrink:0;display:inline-flex;align-items:center;gap:.2rem}',
        '.aipu-dd-upgrade{display:flex;align-items:center;gap:.55rem;width:100%;text-align:left;margin-top:.3rem;padding:.65rem;border:none;border-radius:.7rem;cursor:pointer;color:#fff;font-size:.8rem;font-weight:700;background:linear-gradient(135deg,#8b5cf6,#6d28d9);box-shadow:0 6px 20px rgba(139,92,255,.35)}',
        '.aipu-dd-upgrade:hover{filter:brightness(1.08)}',
        '.aipu-dd-upgrade svg{width:1rem;height:1rem;flex-shrink:0}',
        // exit popup (mirrors aipu-lib-upgrade)
        '#aipu-exit-overlay{position:fixed;inset:0;z-index:100000;display:none;align-items:center;justify-content:center;padding:1.25rem;background:rgba(4,4,10,.85);backdrop-filter:blur(12px);animation:omniGenFadeIn .3s ease}',
        '#aipu-exit-card{position:relative;max-width:30rem;width:100%;border-radius:1.5rem;overflow:hidden;border:1px solid rgba(139,92,255,.25);background:linear-gradient(160deg,rgba(28,18,52,.98) 0%,rgba(10,8,20,.99) 100%);box-shadow:0 32px 80px rgba(0,0,0,.7),0 0 60px rgba(139,92,255,.14),inset 0 1px 0 rgba(255,255,255,.06)}',
        '.aipu-exit-banner{position:relative;height:7rem;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,rgba(139,92,255,.55),rgba(56,189,248,.35))}',
        '.aipu-exit-banner-emoji{font-size:3rem;filter:drop-shadow(0 6px 14px rgba(0,0,0,.4))}',
        '.aipu-exit-inner{position:relative;padding:1.5rem 1.6rem 1.4rem;text-align:center}',
        '.aipu-exit-kicker{display:inline-flex;align-items:center;gap:.4rem;font-size:.66rem;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#fcd34d;background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.28);padding:.3rem .7rem;border-radius:999px;margin-bottom:.85rem}',
        '#aipu-exit-title{font-size:1.45rem;font-weight:800;color:#fafafa;margin:0;line-height:1.25}',
        '#aipu-exit-sub{font-size:.9rem;color:rgba(212,212,216,.92);margin:.6rem 0 0;line-height:1.5}',
        '.aipu-exit-price{display:flex;align-items:baseline;justify-content:center;gap:.5rem;margin:1.1rem 0}',
        '.aipu-exit-price .now{font-size:2rem;font-weight:800;color:#c4b5fd}',
        '.aipu-exit-price .per{font-size:.85rem;color:rgba(161,161,170,.9)}',
        '#aipu-exit-cta{display:block;width:100%;padding:1rem 1.25rem;border:none;border-radius:.9rem;font-size:1rem;font-weight:800;color:#fff;cursor:pointer;background:linear-gradient(135deg,#8b5cf6,#6d28d9);box-shadow:0 10px 30px rgba(139,92,255,.45);transition:transform .15s,box-shadow .15s;animation:omniExitPulse 2s ease-in-out infinite}',
        '#aipu-exit-cta:hover{transform:translateY(-2px);box-shadow:0 14px 38px rgba(139,92,255,.55)}',
        '.aipu-exit-foot{font-size:.72rem;color:rgba(161,161,170,.8);margin:.75rem 0 0}',
        '#aipu-exit-close{position:absolute;top:.7rem;right:.7rem;z-index:2;width:2rem;height:2rem;border:none;border-radius:.5rem;background:rgba(0,0,0,.35);color:#fff;cursor:pointer;font-size:1.2rem;line-height:1}',
        '#aipu-exit-close:hover{background:rgba(0,0,0,.55)}',
    ...
    var openDd = null;
    function closeDd() {
      if (openDd) { openDd.menu.classList.remove('aipu-dd-open'); openDd = null; }
    }
    document.addEventListener('click', function (e) {
    ...
    function attachDropdown(trigger, build) {
      if (trigger.getAttribute('data-aipu-dd') === '1') return;
      trigger.setAttribute('data-aipu-dd', '1');
      trigger.style.cursor = 'pointer';
      var menu = document.createElement('div');
      menu.className = 'aipu-dd';
      ...
      trigger.addEventListener('click', function (e) {
        ...
        menu.classList.add('aipu-dd-open');
    ...
    (function initModelDropdown() {
      ...
      function buildMenu(trigger, currentName) {
        return function (menu) {
          var html = '<div class="aipu-dd-head">Model · ' + studio.label + '</div>';
          studio.models.forEach(function (name, i) {
            var sel = name === currentName ? ' aipu-dd-sel' : '';
            var premium = i >= freeCount;
            html += '<button type="button" class="aipu-dd-item' + sel + '" data-model="' + name + '" data-premium="' + (premium ? '1' : '0') + '">' +
              '<span class="aipu-dd-check">✓</span>' +
              '<span class="aipu-dd-name">' + name + '</span>' +
              (premium ? '<span class="aipu-dd-lock">' + lockSvg + 'PRO</span>' : '') +
            '</button>';
          });
          html += '<button type="button" class="aipu-dd-upgrade" data-upgrade="1">' + lockSvg + 'Upgrade to see 140+ more models</button>';
          menu.innerHTML = html;
          menu.addEventListener('click', function (e) {
            var up = e.target.closest('[data-upgrade]');
            if (up) { goCheckout(); return; }
            var item = e.target.closest('.aipu-dd-item');
            if (!item) return;
            if (item.getAttribute('data-premium') === '1') { goCheckout(); return; }
            menu.querySelectorAll('.aipu-dd-item').forEach(function (el) { el.classList.remove('aipu-dd-sel'); });
            item.classList.add('aipu-dd-sel');
    ...
    (function initChipDropdowns() {
      ...
      function buildChipMenu(trigger, set, current) {
        return function (menu) {
          var html = '<div class="aipu-dd-head">' + set.head + '</div>';
          set.free.forEach(function (v) {
            var sel = v === current ? ' aipu-dd-sel' : '';
            html += '<button type="button" class="aipu-dd-item' + sel + '" data-val="' + v + '" data-premium="0"><span class="aipu-dd-check">✓</span><span class="aipu-dd-name">' + v + '</span></button>';
          });
          set.premium.forEach(function (v) {
            html += '<button type="button" class="aipu-dd-item" data-val="' + v + '" data-premium="1"><span class="aipu-dd-check">✓</span><span class="aipu-dd-name">' + v + '</span><span class="aipu-dd-lock">' + lockSvg + 'PRO</span></button>';
          });
          html += '<button type="button" class="aipu-dd-upgrade" data-upgrade="1">' + lockSvg + 'Unlock all options — upgrade</button>';
          menu.innerHTML = html;
          menu.addEventListener('click', function (e) {
      ...
      document.querySelectorAll('button[aria-haspopup="menu"]').forEach(function (btn) {
        ...
        if (btn.getAttribute('data-aipu-dd') === '1') return;
        var label = btn.textContent.replace(/\s+/g, ' ').trim();
        var set = null;
    ...
    (function initExitPopup() {
      ...
      var overlay = document.createElement('div');
      overlay.id = 'aipu-exit-overlay';
      overlay.innerHTML =
        '<div id="aipu-exit-card">' +
          '<button type="button" id="aipu-exit-close" aria-label="Close">&times;</button>' +
          '<div class="aipu-exit-banner"><span class="aipu-exit-banner-emoji">🍌</span></div>' +
          '<div class="aipu-exit-inner">' +
            '<span class="aipu-exit-kicker">⏳ Limited-time launch offer</span>' +
            '<p id="aipu-exit-title">Claim 250 Free Gens of Nano Banana</p>' +
            '<p id="aipu-exit-sub">Don\'t leave empty-handed — start creating across 140+ top models today.</p>' +
            '<div class="aipu-exit-price"><span class="now">$14.99</span><span class="per">to get started</span></div>' +
            '<button type="button" id="aipu-exit-cta">Claim now →</button>' +
            '<p class="aipu-exit-foot">No commitment · cancel anytime</p>' +
          '</div>' +
        '</div>';
      document.body.appendChild(overlay);
      ...
      overlay.querySelector('#aipu-exit-cta').addEventListener('click', goCheckout);
      overlay.querySelector('#aipu-exit-close').addEventListener('click', hide);
    ...
    (function initSocialProof() {
      ...
      function nextMessage() {
        ...
        if (roll < 0.8) {
          return { text: '<b>' + pad(900 + Math.floor(Math.random() * 1500)) + ' creations</b> in the last hour', sub: 'across the AIPU studio' };
        }
  ...
  if (/home\.html|about\.html|gpt-library\.html|prompt-library\.html/.test(window.location.pathname)) {
